...
`coverage/r5900_isa.yaml` contains 54 encodings in exact roadmap order: 47
complete operations and 7 pending doubleword or dual-HI/LO
...
SPECIAL function `0x2f` subtracts `rt[63:0]` from `rs[63:0]` modulo 64,
writes the low destination doubleword, preserves old `rd[127:64]`, and cannot
raise integer overflow. Directed tests cover zero, one, signed-boundary bit
patterns, borrow and wrap; both destination aliases; identical and zero
sources; ignored source upper lanes; GPR zero; PC wrap; reserved `sa`; and exact
events. A 524-case seeded differential stream independently checks the
pre-commit candidate, PC, and complete GPR state after every operation.

## R5900 MULT

SPECIAL function `0x18` multiplies signed `rs[31:0]` and `rt[31:0]`. The
64-bit product is split into two words and each word is independently
sign-extended: bits 63:32 become primary HI and bits 31:0 become primary LO.
A nonzero optional `rd` also receives LO in its low scalar lane while preserving
old destination bits 127:64; `rd == 0` suppresses only that GPR write. Directed
tests cover signed extrema, product-half sign boundaries, all aliases, GPR zero,
...
SPECIAL function `0x1a` divides signed `rs[31:0]` by signed `rt[31:0]`, puts
the sign-extended remainder in primary HI, and puts the sign-extended quotient
in primary LO without a GPR write. Directed tests cover all operand-sign
pairings, truncation toward zero, signed remainders, ignored source upper lanes,
source aliasing, the signed overflow pair, positive/negative/zero dividends at
divisor zero, PC wrap, HI1/LO1 isolation, exact events, and nonzero reserved
`rd` or `sa` rejection. A 524-case seeded differential stream compares PC,
every GPR, HI, LO, HI1, and LO1 after every operation.

## R5900 DIVU

SPECIAL function `0x1b` divides unsigned `rs[31:0]` by unsigned `rt[31:0]`,
then independently sign-extends the 32-bit remainder into primary HI and the
32-bit quotient into primary LO without a GPR write. Directed tests cover
unsigned extrema, quotient and remainder sign-bit boundaries, results that
differ from DIV, ignored source upper lanes, source aliasing, five dividend
classes at divisor zero, PC wrap, HI1/LO1 isolation, exact events, and nonzero
reserved `rd` or `sa` rejection. A 524-case seeded differential stream compares
PC, every GPR, HI, LO, HI1, and LO1 after every operation.

## R5900 MFHI

SPECIAL function `0x10` copies all 64 primary HI bits into `rd[63:0]` while
preserving `rd[127:64]` and leaving HI, LO, HI1, and LO1 unchanged. Directed
tests cover zero, word and doubleword sign boundaries, asymmetric full-width
values, preserved destination upper lanes, destination zero suppression, PC
wrap, exact candidate/writeback/retirement events, and populated reserved `rs`,
`rt`, or `sa` rejection. A 520-case seeded differential stream varies both HI
and `rd`, then compares PC, every GPR, and all four accumulator fields after
every operation.

## R5900 MFLO

SPECIAL function `0x12` copies all 64 primary LO bits into `rd[63:0]` while
preserving `rd[127:64]` and leaving HI, LO, HI1, and LO1 unchanged. Directed
tests cover zero, word and doubleword sign boundaries, asymmetric full-width
values, preserved destination upper lanes, destination zero suppression, PC
wrap, exact candidate/writeback/retirement events, and populated reserved `rs`,
`rt`, or `sa` rejection. A 520-case seeded differential stream varies both LO
and `rd`, then compares PC, every GPR, and all four accumulator fields after
every operation.

## R5900 MTHI

SPECIAL function `0x11` copies `rs[63:0]` into primary HI while ignoring GPR
bits 127:64 and leaving every GPR plus LO, HI1, and LO1 unchanged. Directed
tests cover zero, word and doubleword sign boundaries, asymmetric full-width
values, changing upper GPR lanes, source zero, PC wrap, exact HI-write and
retirement events, and populated reserved `rt`, `rd`, or `sa` rejection. A
520-case seeded differential stream varies both the complete 128-bit source GPR
and `rs`, then compares PC, every GPR, and all four accumulator fields after
every operation.

## R5900 MTLO

SPECIAL function `0x13` copies `rs[63:0]` into primary LO while ignoring GPR
bits 127:64 and leaving every GPR plus HI, HI1, and LO1 unchanged. Directed
tests cover zero, word and doubleword sign boundaries, asymmetric full-width
values, changing upper GPR lanes, source zero, PC wrap, exact LO-write and
retirement events, and populated reserved `rt`, `rd`, or `sa` rejection. A
520-case seeded differential stream varies both the complete 128-bit source GPR
and `rs`, then compares PC, every GPR, and all four accumulator fields after
every operation.
...
MMI opcode `0x1c`, function `0x18` multiplies signed `rs[31:0]` and `rt[31:0]`,
independently sign-extends the high and low product words into secondary HI1 and
LO1, and preserves primary HI and LO. A nonzero optional `rd` receives LO1 in
bits 63:0 while retaining bits 127:64. Directed tests cover signed extrema,
opposite-sign products, independent half-extension boundaries, ignored source
upper lanes, destination/source aliases, destination zero, PC wrap, primary
state isolation, exact secondary write and retirement events, and nonzero
reserved `sa` rejection. A 524-case seeded differential stream compares PC,
every GPR, and all four accumulator fields after every operation.

## R5900 MULTU1

MMI opcode `0x1c`, function `0x19` multiplies unsigned `rs[31:0]` and
`rt[31:0]`, independently sign-extends the high and low product words into
secondary HI1 and LO1, and preserves primary HI and LO. A nonzero optional
`rd` receives LO1 in bits 63:0 while retaining bits 127:64. Directed tests
cover unsigned extrema, independent half-extension boundaries, ignored source
upper lanes, destination/source aliases, destination zero, PC wrap, primary
state isolation, exact secondary write and retirement events, and nonzero
reserved `sa` rejection. A 524-case seeded differential stream compares PC,
every GPR, and all four accumulator fields after every operation.
...
MMI opcode `0x1c`, function `0x1a` divides signed `rs[31:0]` by signed
`rt[31:0]`, writes the remainder to HI1 and quotient to LO1, and preserves
primary HI/LO plus every GPR. The result truncates toward zero; a zero divisor
returns the sign-extended dividend in HI1 and one or negative one in LO1 based
on dividend sign, while `INT_MIN / -1` returns zero remainder and `INT_MIN`
...
MMI opcode `0x1c`, function `0x10` copies all 64 bits of secondary HI1 into
destination bits 63:0 while preserving bits 127:64. Destination zero is
suppressed by centralized writeback, and every accumulator field remains
unchanged. Directed tests cover eight full-width boundary classes, destination
zero, PC wrap, upper-lane preservation, exact writeback and retirement events,
and nonzero reserved `rs`, `rt`, or `sa` rejection. A 520-case seeded
differential stream varies HI1 and the destination, then compares PC, every GPR,
and all four accumulator fields after every operation.
