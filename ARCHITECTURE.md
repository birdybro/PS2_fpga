...
connects primary and implemented secondary HI/LO writes from execute. The block
has no reset input;
...
SLLV, SRLV, SRAV, DSLLV, DSRLV, DSRAV, DSLL, DSRL, DSRA, DSLL32, DSRL32, DSRA32, ADDU, DADDU, SUBU, DSUBU, MULT, MULTU, DIV, DIVU, MFHI, MFLO, MTHI, MTLO, MMI MULT1, MMI MULTU1, MMI DIV1, MMI DIVU1, MMI MFHI1, AND, OR, XOR, NOR,
...
to be clear; MMI DIV1 and DIVU1 require both reserved `rd` and `sa` to be clear,
while MMI MFHI1 requires reserved `rs`, `rt`, and `sa` to be clear.
Every unsupported word maps to no
...
optional destination rules to its product. DIV divides the signed low word of
`rs` by the signed low word of `rt`, places the sign-extended remainder in
primary HI and the sign-extended quotient in primary LO, and never writes a
GPR. Quotients truncate toward zero. The signed overflow pair produces zero in
HI and `0xffffffff80000000` in LO; division by zero retains the signed dividend
in HI and writes one for a negative dividend or all ones otherwise to LO.
DIVU instead treats both low words as unsigned and independently sign-extends
the 32-bit remainder and quotient into primary HI and LO. A zero divisor writes
the sign-extended dividend word to HI and all ones to LO. Neither divide writes
...
all 64 bits of primary LO. MTHI copies `rs[63:0]` into primary HI, ignores
`rs[127:64]`, and preserves every GPR plus LO, HI1, and LO1.
...
signed extension of each product half to the secondary path. DIV1 applies DIV's
signed low-word quotient, remainder, overflow, and divisor-zero rules to HI1
and LO1 without changing primary HI/LO. DIVU1 applies DIVU's unsigned low-word
quotient, remainder, and divisor-zero rules to HI1 and LO1 with the same primary
state isolation. MFHI1 copies all 64 bits of secondary HI1 into `rd[63:0]`,
preserves `rd[127:64]`, suppresses destination zero, and leaves all four
multiply/divide registers unchanged. AND combines the
