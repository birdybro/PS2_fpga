...
SLLV, SRLV, SRAV, DSLL, DSRL, ADDU, SUBU, AND, OR, XOR, NOR, SLT, and SLTU; and primary-opcode LUI, ORI, ANDI, XORI, ADDIU, SLTI, and SLTIU
...
and exact instruction. DSLL and DSRL shift the complete low 64-bit source lane
left or logically right by the immediate count, truncate at 64 bits, and
preserve destination bits 127:64 without word sign extension. LUI places its immediate in word bits 31:16, applies the
...
extension rules to the low words of two GPR sources; SUBU applies them to
nontrapping modulo-32-bit subtraction. AND combines the full low 64-bit scalar
