...
R5900_HILO_STATE := rtl/ee/r5900/r5900_hilo_state.sv
...
lint: structure venv ## Run HDL, Python, YAML, whitespace, and hygiene checks.
	...
	$(VERILATOR) --lint-only -Wall --top-module r5900_hilo_state \
		$(VERILATOR_FLAGS) rtl/ee/r5900/r5900_types_pkg.sv $(R5900_HILO_STATE)
	...
	$(VERILATOR) --lint-only -Wall --assert --top-module r5900_nop_top \
		$(VERILATOR_FLAGS) rtl/ee/r5900/r5900_types_pkg.sv \
	...
	$(VERILATOR) --lint-only -Wall --assert --timing --top-module ps2_sim_top \
		$(VERILATOR_FLAGS) $(RTL_SOURCES) $(SIM_SOURCES)
	$(VERILATOR) --lint-only -Wall --assert --timing -GR5900_FETCH_ENABLE=1 \
		--top-module ps2_sim_top $(VERILATOR_FLAGS) $(RTL_SOURCES) $(SIM_SOURCES)
	$(VERILATOR) --lint-only -Wall --assert --timing -GR5900_CORE_ENABLE=1 \
		--top-module ps2_sim_top $(VERILATOR_FLAGS) $(RTL_SOURCES) $(SIM_SOURCES)
