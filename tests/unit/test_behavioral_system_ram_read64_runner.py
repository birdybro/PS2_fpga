"""Pytest orchestration for behavioral RAM aligned 64-bit reads."""
...
@pytest.mark.unit
def test_behavioral_system_ram_read64_with_verilator() -> None:
    """Verify 64-bit little-endian reads, alignment, bounds, and backpressure."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_read64"
    results_path = build_root / "results" / "cocotb-behavioral-system-ram-read64.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_read64",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
