"""Directed architectural tests for R5900 DSLL32."""
...
def encode(destination: int, source: int, count: int) -> int:
    return (source << 16) | (destination << 11) | (count << 6) | 0x3C
...
async def execute(dut, destination: int, source: int, count: int, old_rd: int) -> int:
    ...
    expected_scalar = ((source_value & SCALAR_MASK) << (count + 32)) & SCALAR_MASK
    ...
    await Timer(1, unit="ns")
    assert int(dut.operation_o.value) == OPERATION_DSLL32
...
@cocotb.test()
async def test_dsll32_effective_counts_and_upper_lane(dut) -> None:
    """Cover effective shifts 32, 33, 62, and 63 without count wrap."""
    ...
    for count, scalar, result in (
        ...
        (30, 3, 0xC000_0000_0000_0000),
        (31, 1, 0x8000_0000_0000_0000),
    ):
...
@cocotb.test()
async def test_dsll32_alias_zero_and_pc_wrap(dut) -> None:
