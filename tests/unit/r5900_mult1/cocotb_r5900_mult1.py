"""Directed architectural tests for R5900 MULT1."""
...
LO_SEED = 0x5555_6666_7777_8888
INITIAL_HI1 = 0x3333_4444_5555_6666
INITIAL_LO1 = 0x7777_8888_9999_AAAA
RESERVED_TEST_PC = 0x3000


def encode_mult1(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (
        (0x1C << 26)
        | (source_a << 21)
        | (source_b << 16)
...
def expected_product(source_a_value: int, source_b_value: int) -> tuple[int, int]:
    product = (signed_word(source_a_value) * signed_word(source_b_value)) & SCALAR_MASK
    hi_word = (product >> 32) & WORD_MASK
    lo_word = product & WORD_MASK
    hi = hi_word | ((SCALAR_MASK ^ WORD_MASK) if hi_word & (1 << 31) else 0)
    lo = lo_word | ((SCALAR_MASK ^ WORD_MASK) if lo_word & (1 << 31) else 0)
    return hi, lo
...
async def execute_mult1(
    ...
    instruction = encode_mult1(destination, source_a, source_b)
    ...
    assert int(dut.operation_o.value) == OPERATION_MULT1
    ...
    assert (int(dut.hi1_o.value), int(dut.lo1_o.value)) == (expected_hi, expected_lo)
...
@cocotb.test()
async def test_r5900_mult1_signed_extrema_and_independent_half_extension(dut) -> None:
    await initialize(dut, 0x1000)
    cases = (
        (0x0000_0000, 0xFFFF_FFFF, 0, 0),
        (0x0000_0001, 0x0000_0001, 0, 1),
        (0xFFFF_FFFF, 0x0000_0001, SCALAR_MASK, SCALAR_MASK),
        (0xFFFF_FFFF, 0xFFFF_FFFF, 0, 1),
        (0x7FFF_FFFF, 0x0000_0002, 0, 0xFFFF_FFFF_FFFF_FFFE),
        (0x8000_0000, 0x0000_0001, SCALAR_MASK, 0xFFFF_FFFF_8000_0000),
    ...
    for source_a_word, source_b_word, expected_hi, expected_lo in cases:
        ...
        assert await execute_mult1(dut, (5, 3, 4), destination, (source_a, source_b)) == (
            expected_hi,
            expected_lo,
        )
...
@cocotb.test()
async def test_r5900_mult1_optional_destination_zero_still_updates_hilo1(dut) -> None:
    ...
    await execute_mult1(dut, (0, 8, 9), 0, (source_a, source_b))


@cocotb.test()
async def test_r5900_mult1_optional_destination_aliases_source_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    await seed_gpr(dut, 31, source)
    await execute_mult1(dut, (31, 31, 31), source, (source, source))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_mult1_rejects_nonzero_reserved_shift_field(dut) -> None:
    await initialize(dut, RESERVED_TEST_PC)
    instruction = encode_mult1(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
