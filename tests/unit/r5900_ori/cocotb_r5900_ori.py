"""Directed architectural tests for R5900 ORI."""
...
def encode_ori(destination: int, source: int, immediate: int) -> int:
    return (0x0D << 26) | (source << 21) | (destination << 16) | immediate


def expected_ori(old_destination: int, source_value: int, immediate: int) -> int:
...
async def execute_ori(
    ...
    expected = expected_ori(old_destination, source_value, immediate)
...
@cocotb.test()
async def test_r5900_ori_immediate_boundaries_and_independent_operands(dut) -> None:
    await initialize(dut, 0x1000)
    source_value = 0xFFFF_FFFF_FFFF_FFFF_1234_5678_9ABC_0000
    ...
    for immediate in (0, 1, 0x7FFF, 0x8000, 0xFFFF):
        ...
        assert await execute_ori(dut, 5, 4, immediate, (old_destination, source_value)) == expected
        assert int(dut.source_rs_value_o.value) == source_value


@cocotb.test()
async def test_r5900_ori_source_destination_alias_and_zero_source(dut) -> None:
    await initialize(dut, 0x2000)
    aliased = 0xCAFE_BABE_1234_5678_FEDC_BA98_7654_0000
    await seed_gpr(dut, 31, aliased)
    await execute_ori(dut, 31, 31, 0xFFFF, (aliased, aliased))
    destination = 0x1111_2222_3333_4444_AAAA_BBBB_CCCC_DDDD
    await seed_gpr(dut, 7, destination)
    await execute_ori(dut, 7, 0, 0x8000, (destination, 0))


@cocotb.test()
async def test_r5900_ori_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_value = 0x9999_AAAA_BBBB_CCCC_0123_4567_89AB_CDEF
    await seed_gpr(dut, 9, source_value)
    await execute_ori(dut, 0, 9, 0xFFFF, (0, source_value))
