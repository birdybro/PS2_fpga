"""Directed architectural tests for R5900 SLTU."""
...
def encode_sltu(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (source_a << 21) | (source_b << 16) | (destination << 11) | (shift << 6) | 0x2B


def expected_sltu(old_destination: int, source_a_value: int, source_b_value: int) -> int:
    result = int((source_a_value & SCALAR_MASK) < (source_b_value & SCALAR_MASK))
...
async def execute_sltu(
    ...
    instruction = encode_sltu(destination, source_a, source_b)
    expected = expected_sltu(old_destination, source_a_value, source_b_value)
    ...
    assert int(dut.operation_o.value) == OPERATION_SLTU
...
@cocotb.test()
async def test_r5900_sltu_unsigned_scalar_boundaries_and_result_width(dut) -> None:
    ...
    for source_a_scalar, source_b_scalar, expected_scalar in cases:
        ...
        assert (
            await execute_sltu(dut, 5, 3, 4, (old_destination, source_a_value, source_b_value))
            == destination_upper | expected_scalar
        )


@cocotb.test()
async def test_r5900_sltu_handles_destination_aliases_and_equal_sources(dut) -> None:
    ...
    await execute_sltu(dut, 31, 31, 30, (source_a, source_a, source_b))
    await seed_gpr(dut, 31, source_a)
    await seed_gpr(dut, 30, source_b)
    await execute_sltu(dut, 30, 31, 30, (source_b, source_a, source_b))
    ...
    await execute_sltu(dut, 29, 28, 28, (old_destination, source_b, source_b))


@cocotb.test()
async def test_r5900_sltu_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_a = 0x9999_AAAA_BBBB_CCCC_0000_0000_0000_0001
    source_b = 0x1111_2222_3333_4444_FFFF_FFFF_FFFF_FFFF
    await seed_gpr(dut, 8, source_a)
    await seed_gpr(dut, 9, source_b)
    await execute_sltu(dut, 0, 8, 9, (0, source_a, source_b))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_sltu_rejects_nonzero_reserved_shift_field(dut) -> None:
    ...
    instruction = encode_sltu(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
