"""Directed architectural tests for R5900 DSUBU."""
...
def encode_dsubu(destination: int, minuend: int, subtrahend: int, shift: int = 0) -> int:
    return (minuend << 21) | (subtrahend << 16) | (destination << 11) | (shift << 6) | 0x2F


def expected_dsubu(old_destination: int, minuend_value: int, subtrahend_value: int) -> int:
    scalar = ((minuend_value & SCALAR_MASK) - (subtrahend_value & SCALAR_MASK)) & SCALAR_MASK
...
async def execute_dsubu(
    ...
    expected = expected_dsubu(old_destination, minuend_value, subtrahend_value)
...
@cocotb.test()
async def test_r5900_dsubu_doubleword_borrow_and_wrap_boundaries(dut) -> None:
    ...
    for minuend_scalar, subtrahend_scalar, expected_scalar in cases:
        minuend_value = 0xFFFF_FFFF_FFFF_FFFF_0000_0000_0000_0000 | minuend_scalar
        subtrahend_value = 0xAAAA_AAAA_AAAA_AAAA_0000_0000_0000_0000 | subtrahend_scalar
...
@cocotb.test()
async def test_r5900_dsubu_handles_both_destination_aliases_and_identical_sources(dut) -> None:
    ...
    await execute_dsubu(dut, 31, 31, 30, (minuend, minuend, subtrahend))
    await seed_gpr(dut, 31, minuend)
    await seed_gpr(dut, 30, subtrahend)
    await execute_dsubu(dut, 30, 31, 30, (subtrahend, minuend, subtrahend))
    ...
    await execute_dsubu(dut, 29, 28, 28, (old_destination, subtrahend, subtrahend))


@cocotb.test()
async def test_r5900_dsubu_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    minuend = 0x9999_AAAA_BBBB_CCCC_0000_0000_0000_0000
    subtrahend = 0x1111_2222_3333_4444_0000_0000_0000_0001
    await seed_gpr(dut, 8, minuend)
    await seed_gpr(dut, 9, subtrahend)
    await execute_dsubu(dut, 0, 8, 9, (0, minuend, subtrahend))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_dsubu_rejects_nonzero_reserved_shift_field(dut) -> None:
    ...
    instruction = encode_dsubu(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
