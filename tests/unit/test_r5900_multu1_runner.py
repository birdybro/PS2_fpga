"""Pytest orchestration for directed R5900 MULTU1 tests."""
...
@pytest.mark.unit
def test_r5900_multu1_directed() -> None:
    sources = [
        ...
        TESTBENCH_DIR / "r5900_multu1_top.sv",
    ]
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_multu1_directed"
    ...
    result = runner.test(
        test_module="cocotb_r5900_multu1",
        hdl_toplevel="r5900_multu1_top",
        build_dir=build_dir,
        test_dir=TESTBENCH_DIR,
        results_xml=str(build_root / "results/cocotb-r5900-multu1-directed.xml"),
    )
