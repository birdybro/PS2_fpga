"""Directed architectural tests for R5900 32-bit SRL."""
...
OPERATION_SLL = 2
OPERATION_SRL = 3
LOGICAL_SHIFT_RESULT = 0x0123_4567_89AB_CDEF_0000_0000_4000_0000


def encode_srl(destination: int, source: int, shift_amount: int) -> int:
    """Build a canonical SPECIAL SRL instruction independently of the model."""
    return (source << 16) | (destination << 11) | (shift_amount << 6) | 2


def srl_result(source: int, old_destination: int, shift_amount: int) -> int:
    """Form the expected R5900 destination with explicit word and lane operations."""
    word = (source & WORD_MASK) >> shift_amount
...
async def execute_srl(
    ...
    *,
    ...
    """Check one complete SRL transaction and return its expected destination."""
    destination = (instruction >> 11) & 0x1F
    shift_amount = (instruction >> 6) & 0x1F
    expected = srl_result(expected_source, old_destination, shift_amount)
...
@cocotb.test()
async def test_r5900_srl_is_logical_and_preserves_destination_upper_lane(dut) -> None:
    """Ignore source high bits, shift in zeros, and retain rd bits 127:64."""
    ...
    result = await execute_srl(
        dut,
        instruction=encode_srl(5, 3, 1),
        expected_source=source,
        old_destination=destination,
    )
    assert result == LOGICAL_SHIFT_RESULT


@cocotb.test()
async def test_r5900_srl_covers_zero_one_thirty_and_thirty_one_counts(dut) -> None:
    """Cover all boundary counts including sign extension after a zero-count shift."""
    ...
    for shift_amount, low_word, expected_scalar in cases:
        ...
        result = await execute_srl(
            dut,
            instruction=encode_srl(9, 8, shift_amount),
            expected_source=source,
            old_destination=destination,
        )
...
@cocotb.test()
async def test_r5900_srl_reads_before_aliased_destination_write(dut) -> None:
    """Use the original source word and upper lane when rd equals rt."""
    await initialize(dut, 0x3000)
    original = 0xCAFE_BABE_1234_5678_8765_4321_8000_0010
    await seed_gpr(dut, 7, original)
    await execute_srl(
        dut,
        instruction=encode_srl(7, 7, 4),
...
@cocotb.test()
async def test_r5900_srl_to_zero_retires_without_architectural_writeback(dut) -> None:
    """Complete legal SRL while the centralized boundary protects GPR zero."""
    await initialize(dut, 0xFFFF_FFFC)
    source = 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_8000_0000
    await seed_gpr(dut, 4, source)
    await execute_srl(
        dut,
        instruction=encode_srl(0, 4, 31),
        expected_source=source,
        old_destination=0,
    )
...
@cocotb.test()
async def test_r5900_srl_encoding_is_distinct_and_rejects_reserved_rs(dut) -> None:
    """Keep SLL and SRL operation IDs distinct and reject nonzero reserved rs."""
