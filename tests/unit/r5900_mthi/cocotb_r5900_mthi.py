"""Directed architectural tests for R5900 MTHI."""
...
OPERATION_NONE = 0
OPERATION_MTHI = 41
INITIAL_HI = 0x1111_2222_3333_4444
...
def encode_mthi(source: int, reserved_rt_rd: int = 0, reserved_sa: int = 0) -> int:
    """Encode MTHI while allowing explicit reserved-field negative tests."""
    return (source << 21) | (reserved_rt_rd << 11) | (reserved_sa << 6) | 0x11
...
async def execute_mthi(dut, source: int, source_value: int) -> None:
    """Execute one canonical MTHI and compare all events and architectural state."""
    instruction = encode_mthi(source)
    expected_hi = source_value & SCALAR_MASK if source else 0
    ...
    assert int(dut.operation_o.value) == OPERATION_MTHI
    ...
    assert (int(dut.hi_o.value), int(dut.lo_o.value)) == (expected_hi, LO_SEED)
...
@cocotb.test()
async def test_r5900_mthi_full_width_values_and_ignored_upper_lane(dut) -> None:
    ...
    for scalar in (
        ...
        await execute_mthi(dut, 31, source_value)


@cocotb.test()
async def test_r5900_mthi_source_zero_writes_zero_to_hi(dut) -> None:
    """Read architectural GPR zero and replace a nonzero primary HI with zero."""
    await initialize(dut, 0x2000)
    await execute_mthi(dut, 0, GPR_MASK)


@cocotb.test()
async def test_r5900_mthi_wraps_pc_and_preserves_sibling_accumulators(dut) -> None:
    """Retire at the PC boundary while LO, HI1, and LO1 hold."""
    await initialize(dut, 0xFFFF_FFFC)
    source_value = 0xAAAA_BBBB_CCCC_DDDD_8000_0000_0000_0001
    await seed_gpr(dut, 1, source_value)
    await execute_mthi(dut, 1, source_value)
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_mthi_rejects_nonzero_reserved_fields(dut) -> None:
    ...
    for instruction in (
        encode_mthi(5, reserved_rt_rd=1),
        encode_mthi(5, reserved_rt_rd=1 << 5),
        encode_mthi(5, reserved_rt_rd=0x3FF),
        encode_mthi(5, reserved_sa=1),
        encode_mthi(5, reserved_sa=31),
    ):
        dut.instruction_i.value = instruction
        dut.instruction_valid_i.value = 1
        await Timer(1, unit="ns")
