"""Pytest orchestration for behavioral RAM aligned 64-bit writes."""
...
@pytest.mark.unit
def test_behavioral_system_ram_write64_with_verilator() -> None:
    """Verify aligned writes, all byte enables, bounds, and backpressure."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_write64"
    results_path = build_root / "results" / "cocotb-behavioral-system-ram-write64.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_write64",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
