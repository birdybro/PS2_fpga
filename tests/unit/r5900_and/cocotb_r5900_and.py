"""Directed architectural tests for R5900 AND."""
...
def encode_and(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (source_a << 21) | (source_b << 16) | (destination << 11) | (shift << 6) | 0x24


def expected_and(old_destination: int, source_a_value: int, source_b_value: int) -> int:
...
async def execute_and(
    ...
    instruction = encode_and(destination, source_a, source_b)
    expected = expected_and(old_destination, source_a_value, source_b_value)
    ...
    assert int(dut.operation_o.value) == OPERATION_AND
...
@cocotb.test()
async def test_r5900_and_scalar_patterns_and_lane_boundaries(dut) -> None:
...
@cocotb.test()
async def test_r5900_and_handles_both_destination_aliases_and_identical_sources(dut) -> None:
    ...
    await execute_and(dut, 31, 31, 30, (source_a, source_a, source_b))
    await seed_gpr(dut, 31, source_a)
    await seed_gpr(dut, 30, source_b)
    await execute_and(dut, 30, 31, 30, (source_b, source_a, source_b))
    ...
    await execute_and(dut, 29, 28, 28, (old_destination, source_b, source_b))


@cocotb.test()
async def test_r5900_and_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_a = 0x9999_AAAA_BBBB_CCCC_FFFF_FFFF_FFFF_FFFF
    source_b = 0x1111_2222_3333_4444_0123_4567_89AB_CDEF
    await seed_gpr(dut, 8, source_a)
    await seed_gpr(dut, 9, source_b)
    await execute_and(dut, 0, 8, 9, (0, source_a, source_b))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_and_rejects_nonzero_reserved_shift_field(dut) -> None:
    ...
    instruction = encode_and(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
