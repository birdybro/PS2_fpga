"""Directed architectural tests for R5900 MULTU1."""
...
def encode_multu1(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (
        (0x1C << 26)
        | (source_a << 21)
        | (source_b << 16)
...
async def execute_multu1(
    ...
    instruction = encode_multu1(destination, source_a, source_b)
    ...
    assert int(dut.operation_o.value) == OPERATION_MULTU1
...
@cocotb.test()
async def test_r5900_multu1_unsigned_extrema_and_signed_half_extension(dut) -> None:
    ...
    for source_a_word, source_b_word, expected_hi, expected_lo in cases:
        ...
        assert await execute_multu1(dut, (5, 3, 4), destination, (source_a, source_b)) == (
            expected_hi,
            expected_lo,
        )
...
@cocotb.test()
async def test_r5900_multu1_optional_destination_zero_still_updates_hilo1(dut) -> None:
    ...
    await execute_multu1(dut, (0, 8, 9), 0, (source_a, source_b))


@cocotb.test()
async def test_r5900_multu1_optional_destination_aliases_source_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    await seed_gpr(dut, 31, source)
    await execute_multu1(dut, (31, 31, 31), source, (source, source))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_multu1_rejects_nonzero_reserved_shift_field(dut) -> None:
    await initialize(dut, RESERVED_TEST_PC)
    instruction = encode_multu1(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
