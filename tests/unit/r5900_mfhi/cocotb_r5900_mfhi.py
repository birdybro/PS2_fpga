"""Directed architectural tests for R5900 MFHI."""
...
OPERATION_NONE = 0
OPERATION_MFHI = 39
LO_SEED = 0x1111_2222_3333_4444
HI1_SEED = 0x5555_6666_7777_8888
LO1_SEED = 0x9999_AAAA_BBBB_CCCC
RESERVED_TEST_PC = 0x3000


def encode_mfhi(destination: int, reserved_rs_rt: int = 0, reserved_sa: int = 0) -> int:
    """Encode MFHI while allowing explicit reserved-field negative tests."""
    return (reserved_rs_rt << 16) | (destination << 11) | (reserved_sa << 6) | 0x10
...
async def initialize(dut, start_pc: int, hi_value: int) -> None:
...
async def seed_hi(dut, value: int) -> None:
    """Change primary HI while preserving explicitly supplied sibling fields."""
    dut.seed_hi_i.value = value
    dut.seed_hilo_commit_i.value = 1
    await edge(dut)
    dut.seed_hilo_commit_i.value = 0
    await edge(dut)
...
async def execute_mfhi(dut, destination: int, old_destination: int, hi_value: int) -> int:
    """Execute one canonical MFHI and compare all events and architectural state."""
    instruction = encode_mfhi(destination)
    expected_gpr = (old_destination & (GPR_MASK ^ SCALAR_MASK)) | hi_value
    ...
    assert int(dut.operation_o.value) == OPERATION_MFHI
    ...
    assert (int(dut.hi_o.value), int(dut.lo_o.value)) == (hi_value, LO_SEED)
...
@cocotb.test()
async def test_r5900_mfhi_full_width_values_and_preserved_upper_lane(dut) -> None:
    ...
    for hi_value in (
        ...
        await seed_hi(dut, hi_value)
        await seed_gpr(dut, 31, destination)
        await execute_mfhi(dut, 31, destination, hi_value)


@cocotb.test()
async def test_r5900_mfhi_destination_zero_is_suppressed(dut) -> None:
    """Produce an rd-zero candidate while architectural GPR zero remains fixed."""
    hi_value = 0xFEDC_BA98_7654_3210
    await initialize(dut, 0x2000, hi_value)
    await execute_mfhi(dut, 0, 0, hi_value)


@cocotb.test()
async def test_r5900_mfhi_wraps_pc_without_changing_hilo(dut) -> None:
    """Retire at the PC boundary while all four accumulator fields hold."""
    hi_value = 0x8000_0000_0000_0001
    await initialize(dut, 0xFFFF_FFFC, hi_value)
    destination = 0xAAAA_BBBB_CCCC_DDDD_1111_2222_3333_4444
    await seed_gpr(dut, 1, destination)
    await execute_mfhi(dut, 1, destination, hi_value)
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_mfhi_rejects_nonzero_reserved_fields(dut) -> None:
    """Reject populated rs, rt, or sa while preserving state and PC."""
    hi_value = 0x0123_4567_89AB_CDEF
    await initialize(dut, RESERVED_TEST_PC, hi_value)
    for instruction in (
        encode_mfhi(5, reserved_rs_rt=1),
        encode_mfhi(5, reserved_rs_rt=1 << 5),
        encode_mfhi(5, reserved_rs_rt=0x3FF),
        encode_mfhi(5, reserved_sa=1),
        encode_mfhi(5, reserved_sa=31),
    ):
        dut.instruction_i.value = instruction
        dut.instruction_valid_i.value = 1
        await Timer(1, unit="ns")
