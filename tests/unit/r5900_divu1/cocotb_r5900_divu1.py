"""Directed architectural tests for R5900 unsigned word DIVU1."""
...
def encode_divu1(dividend: int, divisor: int, reserved: int = 0) -> int:
    """Encode DIVU1 while allowing explicit reserved-field negative tests."""
    return (0x1C << 26) | (dividend << 21) | (divisor << 16) | (reserved << 6) | 0x1B
...
def expected_divu1(dividend_value: int, divisor_value: int) -> tuple[int, int]:
...
async def execute_divu1(dut, fields: tuple[int, int], values: tuple[int, int]) -> tuple[int, int]:
    """Execute one DIVU1 and compare every visible event and state update."""
    dividend_register, divisor_register = fields
    instruction = encode_divu1(dividend_register, divisor_register)
    expected_hi, expected_lo = expected_divu1(*values)
    ...
    assert int(dut.operation_o.value) == OPERATION_DIVU1
...
@cocotb.test()
async def test_r5900_divu1_unsigned_extrema_and_result_extension(dut) -> None:
    ...
    for dividend_word, divisor_word in cases:
        ...
        await execute_divu1(dut, (3, 4), (dividend, divisor))


@cocotb.test()
async def test_r5900_divu1_zero_divisor_retains_dividend_word(dut) -> None:
    ...
    for dividend_word in (0, 1, 0x7FFF_FFFF, 0x8000_0000, 0xFFFF_FFFF):
        ...
        assert await execute_divu1(dut, (8, 0), (dividend, 0)) == (
...
@cocotb.test()
async def test_r5900_divu1_source_alias_and_pc_wrap(dut) -> None:
    ...
    assert await execute_divu1(dut, (31, 31), (source, source)) == (0, 1)
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_divu1_rejects_nonzero_reserved_rd_or_sa(dut) -> None:
    """Reject every class of noncanonical DIVU1 reserved-field population."""
    await initialize(dut, RESERVED_TEST_PC)
    for reserved in (1, 1 << 5, 0x3FF):
        instruction = encode_divu1(3, 4, reserved=reserved)
        dut.instruction_i.value = instruction
        dut.instruction_valid_i.value = 1
        await Timer(1, unit="ns")
