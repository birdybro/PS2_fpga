"""Directed architectural tests for R5900 DSLLV."""
...
def encode(destination: int, source: int, shift_register: int) -> int:
    return (shift_register << 21) | (source << 16) | (destination << 11) | 0x14
...
async def execute(
    ...
    expected_scalar = (source_value << (count_value & 0x3F)) & SCALAR_MASK
    ...
    await Timer(1, unit="ns")
    assert int(dut.operation_o.value) == OPERATION_DSLLV
...
@cocotb.test()
async def test_dsllv_masks_six_count_bits_and_preserves_upper_lane(dut) -> None:
...
@cocotb.test()
async def test_dsllv_aliases_zero_and_pc_wrap(dut) -> None:
