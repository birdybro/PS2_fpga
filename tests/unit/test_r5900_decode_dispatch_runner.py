"""Pytest orchestration for R5900 decode dispatch and diagnostic tests."""
...
COCOTB_TEST_COUNT = 10


@pytest.mark.unit
def test_r5900_decode_dispatch_directed() -> None:
    """Verify legal dispatch, exact diagnostics, and illegal execution suppression."""
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_decode_dispatch_directed"
    results_path = build_root / "results" / "cocotb-r5900-decode-dispatch-directed.xml"
    ...
    runner.build(
        sources=[
            ...
            TESTBENCH_DIR / "r5900_decode_dispatch_top.sv",
        ],
    ...
    result = runner.test(
        test_module="cocotb_r5900_decode_dispatch",
        hdl_toplevel="r5900_decode_dispatch_top",
        build_dir=build_dir,
