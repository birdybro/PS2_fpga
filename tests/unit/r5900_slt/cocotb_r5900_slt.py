"""Directed architectural tests for R5900 SLT."""
...
def encode_slt(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (source_a << 21) | (source_b << 16) | (destination << 11) | (shift << 6) | 0x2A
...
def expected_slt(old_destination: int, source_a_value: int, source_b_value: int) -> int:
    result = int(signed_scalar(source_a_value) < signed_scalar(source_b_value))
...
async def execute_slt(
    ...
    instruction = encode_slt(destination, source_a, source_b)
    expected = expected_slt(old_destination, source_a_value, source_b_value)
    ...
    assert int(dut.operation_o.value) == OPERATION_SLT
...
@cocotb.test()
async def test_r5900_slt_signed_scalar_boundaries_and_result_width(dut) -> None:
    ...
    for source_a_scalar, source_b_scalar, expected_scalar in cases:
        ...
        assert (
            await execute_slt(dut, 5, 3, 4, (old_destination, source_a_value, source_b_value))
            == destination_upper | expected_scalar
        )


@cocotb.test()
async def test_r5900_slt_handles_destination_aliases_and_equal_sources(dut) -> None:
    ...
    await execute_slt(dut, 31, 31, 30, (source_a, source_a, source_b))
    await seed_gpr(dut, 31, source_a)
    await seed_gpr(dut, 30, source_b)
    await execute_slt(dut, 30, 31, 30, (source_b, source_a, source_b))
    ...
    await execute_slt(dut, 29, 28, 28, (old_destination, source_b, source_b))


@cocotb.test()
async def test_r5900_slt_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    ...
    await execute_slt(dut, 0, 8, 9, (0, source_a, source_b))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_slt_rejects_nonzero_reserved_shift_field(dut) -> None:
    ...
    instruction = encode_slt(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
