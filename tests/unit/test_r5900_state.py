...
ENCODED_MULTU_EXAMPLE = 0x02F1_F819
...
ENCODED_MTLO_EXAMPLE = 0x03E0_0013
...
def test_r5900_reference_dsrav_masks_six_count_bits_and_sign_fills(
    shift_register_value: int,
    source_scalar: int,
    expected_scalar: int,
) -> None:
    """Use rs low six bits for signed right shift and retain rd upper bits."""
    shift_register = 0xF00D_CAFE_1234_5678_0000_0000_0000_0000 | shift_register_value
    source = 0xDEAD_BEEF_CAFE_F00D_0000_0000_0000_0000 | source_scalar
    destination = 0x0123_4567_89AB_CDEF_AAAA_BBBB_CCCC_DDDD
    state = R5900State.initial(start_pc=PC_MASK - 3)
    state = state.write_gpr(2, shift_register)
    state = state.write_gpr(3, source).write_gpr(5, destination)
    updated = state.step(encode_dsrav(5, 3, 2))
    assert updated.read_gpr(5) == (destination & ~((1 << 64) - 1)) | expected_scalar
    assert updated.read_gpr(2) == shift_register
    assert updated.read_gpr(3) == source
    assert updated.pc == 0
...
@pytest.mark.unit
def test_r5900_reference_dsrav_aliases_zero_and_encoder_validation() -> None:
    """Read all aliases before writeback, protect zero, and validate fields."""
    upper_mask = GPR_MASK ^ ((1 << 64) - 1)
    original = 0xCAFE_BABE_1234_5678_FEDC_BA98_7654_3210
    state = R5900State.initial().write_gpr(2, 4).write_gpr(7, original)
    rd_equals_rt = state.step(encode_dsrav(7, 7, 2))
    assert rd_equals_rt.read_gpr(7) == (original & upper_mask) | 0xFFED_CBA9_8765_4321

    count_and_destination = 0x0123_4567_89AB_CDEF_1111_2222_0000_0021
    state = rd_equals_rt.write_gpr(8, 0xFEDC_BA98_7654_3210)
    state = state.write_gpr(9, count_and_destination)
    rd_equals_rs = state.step(encode_dsrav(9, 8, 9))
    assert rd_equals_rs.read_gpr(9) == (count_and_destination & upper_mask) | 0xFFFF_FFFF_FF6E_5D4C

    rs_equals_rt = rd_equals_rs.write_gpr(10, 3).step(encode_dsrav(11, 10, 10))
    assert rs_equals_rt.read_gpr(11) == 0
    discarded = rs_equals_rt.step(encode_dsrav(0, 7, 2))
    assert discarded.read_gpr(0) == 0
    assert discarded.gprs[1:] == rs_equals_rt.gprs[1:]
    assert encode_dsrav(31, 17, 9) == ENCODED_DSRAV_EXAMPLE
    for fields, error in (
        ((-1, 0, 0), IndexError),
        ((0, GPR_COUNT, 0), IndexError),
        ((0, 0, -1), IndexError),
        ...
        with pytest.raises(error):
            encode_dsrav(*fields)  # type: ignore[arg-type]
...
@pytest.mark.unit
@pytest.mark.parametrize(
    ("source_a_word", "source_b_word", "expected_hi1", "expected_lo1"),
    [
        (0x0000_0000, 0xFFFF_FFFF, 0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
        (0x0000_0001, 0x0000_0001, 0x0000_0000_0000_0000, 0x0000_0000_0000_0001),
        (0xFFFF_FFFF, 0x0000_0001, 0xFFFF_FFFF_FFFF_FFFF, 0xFFFF_FFFF_FFFF_FFFF),
...
def test_r5900_reference_mult1_sign_extends_secondary_product_halves(
    source_a_word: int,
    source_b_word: int,
    expected_hi1: int,
    expected_lo1: int,
) -> None:
    """Multiply signed source words into independent secondary scalar halves."""
    source_a = 0xFFFF_FFFF_FFFF_FFFF_1234_5678_0000_0000 | source_a_word
    source_b = 0xAAAA_AAAA_AAAA_AAAA_8765_4321_0000_0000 | source_b_word
    destination = 0x0123_4567_89AB_CDEF_AAAA_BBBB_CCCC_DDDD
    state = R5900State.initial(
    ...
    updated = state.step(encode_mult1(5, 3, 4))

    expected_gpr = (destination & ~((1 << 64) - 1)) | expected_lo1
    assert updated.read_gpr(5) == expected_gpr
    assert (updated.hi, updated.lo) == (state.hi, state.lo)
    assert (updated.hi1, updated.lo1) == (expected_hi1, expected_lo1)
    assert updated.pc == 0


@pytest.mark.unit
def test_r5900_reference_mult1_optional_rd_alias_zero_and_encoder_validation() -> None:
    """Cover optional rd, source aliasing, GPR zero, and exact MULT1 encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_mult1(31, 31, 31))
    expected = 0xCAFE_BABE_1234_5678_0000_0000_0000_0001
    assert aliased.read_gpr(31) == expected
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (1, 2, 0, 1)
    no_destination = state.step(encode_mult1(0, 31, 31))
    assert no_destination.gprs == state.gprs
    assert (no_destination.hi, no_destination.lo, no_destination.hi1, no_destination.lo1) == (
        1,
        2,
        0,
        1,
    )
    assert encode_mult1(31, 23, 17) == ENCODED_MULT1_EXAMPLE
    for fields in ((-1, 0, 0), (0, GPR_COUNT, 0), (0, 0, GPR_COUNT), (True, 0, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
            encode_mult1(*fields)  # type: ignore[arg-type]
...
@pytest.mark.parametrize(
    ...
    [
        (0x0000_0000, 0xFFFF_FFFF, 0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
        (0x0000_0001, 0x0000_0001, 0x0000_0000_0000_0000, 0x0000_0000_0000_0001),
        (0xFFFF_FFFF, 0x0000_0001, 0x0000_0000_0000_0000, 0xFFFF_FFFF_FFFF_FFFF),
        (0xFFFF_FFFF, 0xFFFF_FFFF, 0xFFFF_FFFF_FFFF_FFFE, 0x0000_0000_0000_0001),
        (0x7FFF_FFFF, 0x0000_0002, 0x0000_0000_0000_0000, 0xFFFF_FFFF_FFFF_FFFE),
        (0x8000_0000, 0x0000_0001, 0x0000_0000_0000_0000, 0xFFFF_FFFF_8000_0000),
        (0x8000_0000, 0xFFFF_FFFF, 0x0000_0000_7FFF_FFFF, 0xFFFF_FFFF_8000_0000),
        (0x8000_0000, 0x8000_0000, 0x0000_0000_4000_0000, 0x0000_0000_0000_0000),
    ],
)
def test_r5900_reference_multu1_sign_extends_unsigned_product_halves(
    source_a_word: int,
    source_b_word: int,
    expected_hi: int,
    expected_lo: int,
) -> None:
    """Multiply unsigned source words into independently extended HI1 and LO1."""
    source_a = 0xFFFF_FFFF_FFFF_FFFF_1234_5678_0000_0000 | source_a_word
    source_b = 0xAAAA_AAAA_AAAA_AAAA_8765_4321_0000_0000 | source_b_word
    destination = 0x0123_4567_89AB_CDEF_AAAA_BBBB_CCCC_DDDD
    state = R5900State.initial(
    ...
    updated = state.step(encode_multu1(5, 3, 4))
...
@pytest.mark.unit
def test_r5900_reference_multu1_optional_rd_alias_zero_and_encoder_validation() -> None:
    """Cover optional rd, source aliasing, GPR zero, and exact MULTU1 encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_multu1(31, 31, 31))
    expected = 0xCAFE_BABE_1234_5678_0000_0000_0000_0001
    assert aliased.read_gpr(31) == expected
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (
        1,
        2,
        0xFFFF_FFFF_FFFF_FFFE,
        1,
    )
    no_destination = state.step(encode_multu1(0, 31, 31))
    assert no_destination.gprs == state.gprs
    assert (no_destination.hi, no_destination.lo) == (state.hi, state.lo)
    assert (no_destination.hi1, no_destination.lo1) == (0xFFFF_FFFF_FFFF_FFFE, 1)
    assert encode_multu1(31, 23, 17) == ENCODED_MULTU1_EXAMPLE
    for fields in ((-1, 0, 0), (0, GPR_COUNT, 0), (0, 0, GPR_COUNT), (True, 0, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
            encode_multu1(*fields)  # type: ignore[arg-type]
...
def test_r5900_reference_multu_sign_extends_unsigned_product_halves(
    source_a_word: int,
    source_b_word: int,
    expected_hi: int,
    expected_lo: int,
) -> None:
    """Multiply unsigned source words while sign-extending each product half."""
    source_a = 0xFFFF_FFFF_FFFF_FFFF_1234_5678_0000_0000 | source_a_word
    source_b = 0xAAAA_AAAA_AAAA_AAAA_8765_4321_0000_0000 | source_b_word
    destination = 0x0123_4567_89AB_CDEF_AAAA_BBBB_CCCC_DDDD
    state = R5900State.initial(
    ...
    updated = state.step(encode_multu(5, 3, 4))
...
@pytest.mark.unit
def test_r5900_reference_multu_optional_rd_alias_zero_and_encoder_validation() -> None:
    """Cover optional rd, source aliasing, GPR zero, and exact MULTU encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_multu(31, 31, 31))
    expected = 0xCAFE_BABE_1234_5678_0000_0000_0000_0001
    assert aliased.read_gpr(31) == expected
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (
        0xFFFF_FFFF_FFFF_FFFE,
    ...
    no_destination = state.step(encode_multu(0, 31, 31))
    assert no_destination.gprs == state.gprs
    assert (no_destination.hi, no_destination.lo) == (0xFFFF_FFFF_FFFF_FFFE, 1)
    assert encode_multu(31, 23, 17) == ENCODED_MULTU_EXAMPLE
    for fields in ((-1, 0, 0), (0, GPR_COUNT, 0), (0, 0, GPR_COUNT), (True, 0, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
            encode_multu(*fields)  # type: ignore[arg-type]
...
@pytest.mark.parametrize(
    ...
    [
        (7, 3, 1, 2),
        (0xFFFF_FFF9, 3, NEGATIVE_ONE_SCALAR, 0xFFFF_FFFF_FFFF_FFFE),
        (7, 0xFFFF_FFFD, 1, 0xFFFF_FFFF_FFFF_FFFE),
        (0xFFFF_FFF9, 0xFFFF_FFFD, NEGATIVE_ONE_SCALAR, 2),
        (0x7FFF_FFFF, 1, 0, 0x0000_0000_7FFF_FFFF),
        (0x8000_0000, 2, 0, 0xFFFF_FFFF_C000_0000),
        (0x8000_0000, 0xFFFF_FFFF, 0, 0xFFFF_FFFF_8000_0000),
        (7, 0, 7, NEGATIVE_ONE_SCALAR),
        (0xFFFF_FFF9, 0, 0xFFFF_FFFF_FFFF_FFF9, 1),
        (0, 0, 0, NEGATIVE_ONE_SCALAR),
    ],
)
def test_r5900_reference_div_updates_signed_word_quotient_and_remainder(
    dividend_word: int,
    divisor_word: int,
    expected_hi: int,
    expected_lo: int,
) -> None:
    """Cover truncation, signed remainders, zero division, and overflow."""
    ...
    updated = state.step(encode_div(23, 17))
...
@pytest.mark.unit
def test_r5900_reference_div_sources_alias_zero_and_encoder_validation() -> None:
    """Cover source aliasing, the zero register, and exact DIV encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFF9
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_div(31, 31))
    assert aliased.gprs == state.gprs
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (0, 1, 3, 4)
    zero_sources = state.step(encode_div(0, 0))
    assert zero_sources.gprs == state.gprs
    assert (zero_sources.hi, zero_sources.lo) == (0, NEGATIVE_ONE_SCALAR)
    assert encode_div(23, 17) == ENCODED_DIV_EXAMPLE
    for fields in ((-1, 0), (0, GPR_COUNT), (True, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
            encode_div(*fields)  # type: ignore[arg-type]
...
def test_r5900_reference_div1_updates_signed_word_quotient_and_remainder(
    dividend_word: int,
    divisor_word: int,
    expected_hi: int,
    expected_lo: int,
) -> None:
    """Cover secondary truncation, signed remainders, zero division, and overflow."""
    dividend = 0xDEAD_BEEF_CAFE_F00D_1234_5678_0000_0000 | dividend_word
    divisor = 0xAAAA_5555_AAAA_5555_8765_4321_0000_0000 | divisor_word
    state = R5900State.initial(
        start_pc=PC_MASK - 3,
    ...
    updated = state.step(encode_div1(23, 17))
...
@pytest.mark.unit
def test_r5900_reference_div1_sources_alias_zero_and_encoder_validation() -> None:
    """Cover secondary source aliasing, the zero register, and exact encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFF9
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_div1(31, 31))
    assert aliased.gprs == state.gprs
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (1, 2, 0, 1)
    zero_sources = state.step(encode_div1(0, 0))
    assert zero_sources.gprs == state.gprs
    assert (zero_sources.hi, zero_sources.lo) == (state.hi, state.lo)
    assert (zero_sources.hi1, zero_sources.lo1) == (0, NEGATIVE_ONE_SCALAR)
    assert encode_div1(23, 17) == ENCODED_DIV1_EXAMPLE
    for fields in ((-1, 0), (0, GPR_COUNT), (True, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
            encode_div1(*fields)  # type: ignore[arg-type]
...
def test_r5900_reference_divu_updates_unsigned_word_quotient_and_remainder(
    dividend_word: int,
    divisor_word: int,
    expected_hi: int,
    expected_lo: int,
) -> None:
    """Cover unsigned division, independent result extension, and divisor zero."""
    dividend = 0xDEAD_BEEF_CAFE_F00D_1234_5678_0000_0000 | dividend_word
    divisor = 0xAAAA_5555_AAAA_5555_8765_4321_0000_0000 | divisor_word
    state = R5900State.initial(
        start_pc=PC_MASK - 3,
    ...
    updated = state.step(encode_divu(23, 17))
...
@pytest.mark.unit
def test_r5900_reference_divu_sources_alias_zero_and_encoder_validation() -> None:
    """Cover source aliasing, the zero register, and exact DIVU encoding."""
    source = 0xCAFE_BABE_1234_5678_FFFF_FFFF_FFFF_FFFF
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4).write_gpr(31, source)
    aliased = state.step(encode_divu(31, 31))
    assert aliased.gprs == state.gprs
    assert (aliased.hi, aliased.lo, aliased.hi1, aliased.lo1) == (0, 1, 3, 4)
    zero_sources = state.step(encode_divu(0, 0))
    assert zero_sources.gprs == state.gprs
    assert (zero_sources.hi, zero_sources.lo) == (0, NEGATIVE_ONE_SCALAR)
    assert encode_divu(23, 17) == ENCODED_DIVU_EXAMPLE
    for fields in ((-1, 0), (0, GPR_COUNT), (True, 0)):
        error = TypeError if any(type(field) is bool for field in fields) else IndexError
        with pytest.raises(error):
...
def test_r5900_reference_mfhi_copies_full_scalar_and_preserves_upper_lane(
    hi_value: int,
) -> None:
    """Transfer every HI width class without changing any accumulator."""
    ...
    updated = state.step(encode_mfhi(31))

    assert updated.read_gpr(31) == (destination & ~((1 << 64) - 1)) | hi_value
...
@pytest.mark.unit
def test_r5900_reference_mfhi_zero_and_encoder_validation() -> None:
    """Suppress destination zero and validate the exact single-field encoder."""
    state = R5900State.initial(hi=0x0123_4567_89AB_CDEF, lo=2, hi1=3, lo1=4)
    updated = state.step(encode_mfhi(0))
    assert updated.gprs == state.gprs
    assert (updated.hi, updated.lo, updated.hi1, updated.lo1) == (state.hi, 2, 3, 4)
    assert encode_mfhi(31) == ENCODED_MFHI_EXAMPLE
...
def test_r5900_reference_mtlo_copies_scalar_and_ignores_upper_lane(scalar: int) -> None:
    """Transfer every scalar width class while preserving all sibling state."""
    source_value = (0xCAFE_BABE_1234_5678 << 64) | scalar
    state = R5900State.initial(
        start_pc=PC_MASK - 3, hi=0x1111, lo=0x2222, hi1=0x3333, lo1=0x4444
    ).write_gpr(31, source_value)
    updated = state.step(encode_mtlo(31))
    assert updated.gprs == state.gprs
    assert (updated.hi, updated.lo, updated.hi1, updated.lo1) == (
        state.hi,
        scalar,
...
@pytest.mark.unit
def test_r5900_reference_mtlo_zero_and_encoder_validation() -> None:
    """Read immutable GPR zero and validate the exact single-field encoder."""
    state = R5900State.initial(hi=1, lo=2, hi1=3, lo1=4)
    updated = state.step(encode_mtlo(0))
    assert updated.gprs == state.gprs
    assert (updated.hi, updated.lo, updated.hi1, updated.lo1) == (1, 0, 3, 4)
    assert encode_mtlo(31) == ENCODED_MTLO_EXAMPLE
    for source in (-1, GPR_COUNT, True):
        error = TypeError if type(source) is bool else IndexError
        with pytest.raises(error):
            encode_mtlo(source)  # type: ignore[arg-type]
