"""Directed architectural tests for R5900 DADDIU."""
...
def encode_daddiu(destination: int, source: int, immediate: int) -> int:
    return (0x19 << 26) | (source << 21) | (destination << 16) | immediate


def expected_daddiu(old_destination: int, source_value: int, immediate: int) -> int:
    signed_immediate = immediate - (1 << 16) if immediate & 0x8000 else immediate
    scalar = ((source_value & SCALAR_MASK) + signed_immediate) & SCALAR_MASK
...
async def execute_daddiu(
    ...
    expected = expected_daddiu(old_destination, source_value, immediate)
...
@cocotb.test()
async def test_r5900_daddiu_signed_immediate_and_doubleword_wrap_boundaries(dut) -> None:
    ...
    for source_scalar, immediate, expected_scalar in cases:
        source_value = 0xDEAD_BEEF_CAFE_F00D_0000_0000_0000_0000 | source_scalar
        ...
        assert (
            await execute_daddiu(dut, 5, 4, immediate, (old_destination, source_value)) == expected
        )


@cocotb.test()
async def test_r5900_daddiu_source_destination_alias_and_zero_source(dut) -> None:
    await initialize(dut, 0x2000)
    aliased = 0xCAFE_BABE_1234_5678_7FFF_FFFF_FFFF_FFFF
    await seed_gpr(dut, 31, aliased)
    await execute_daddiu(dut, 31, 31, 1, (aliased, aliased))
    old_destination = 0x1111_2222_3333_4444_AAAA_BBBB_CCCC_DDDD
    await seed_gpr(dut, 7, old_destination)
    await execute_daddiu(dut, 7, 0, 0x8000, (old_destination, 0))


@cocotb.test()
async def test_r5900_daddiu_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_value = 0x9999_AAAA_BBBB_CCCC_FFFF_FFFF_FFFF_FFFF
    await seed_gpr(dut, 9, source_value)
    await execute_daddiu(dut, 0, 9, 1, (0, source_value))
