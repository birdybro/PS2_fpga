"""Directed aligned 128-bit writes for behavioral system RAM."""
...
TEST_ADDRESS = 16
FULL_STROBE = 0xFFFF
BACKPRESSURE_STROBE = 0xA5A5
BASELINE_BYTES = bytes((0x10 + lane * 7) & 0xFF for lane in range(16))
WRITE_BYTES = bytes((0xF0 - lane * 9) & 0xFF for lane in range(16))
WRITE_DATA = int.from_bytes(WRITE_BYTES, byteorder="little", signed=False)
STROBE_PATTERNS = tuple(
    sorted(
        {
            0x0000,
            FULL_STROBE,
        ...
        | {1 << lane for lane in range(16)}
        | {FULL_STROBE ^ (1 << lane) for lane in range(16)}
    )
)
...
async def backdoor_read_bytes(dut, address: int) -> bytes:
    """Read sixteen adjacent bytes through the simulation backdoor."""
    values = []
...
async def masked_write128(dut, address: int, strobe: int, *, stall_cycles: int = 0) -> None:
    """Issue and consume one byte-enabled 128-bit write."""
...
def expected_bytes(strobe: int) -> bytes:
    """Compute storage results independently from the RTL lane assignments."""
    return bytes(
        WRITE_BYTES[lane] if strobe & (1 << lane) else BASELINE_BYTES[lane] for lane in range(16)
    )


@cocotb.test()
async def aligned_write128_honors_each_byte_lane_and_backpressure(dut) -> None:
    """Exercise lane bases, interactions, boundaries, and malformed addresses."""
    ...
    for address, strobe in ((0, 0x0001), (RAM_SIZE - 16, 0x8000)):
        await backdoor_write_bytes(dut, address, BASELINE_BYTES)
        await masked_write128(dut, address, strobe)
        assert await backdoor_read_bytes(dut, address) == expected_bytes(strobe)

    for invalid_address in (8, RAM_SIZE):
        dut.req_valid_i.value = 1
        dut.req_write_i.value = 1
        dut.req_addr_i.value = invalid_address
        dut.req_size_i.value = 4
        dut.req_wstrb_i.value = FULL_STROBE
        await Timer(1, unit="ns")
