"""Directed architectural tests for R5900 signed word DIV1."""
...
def encode_div1(dividend: int, divisor: int, reserved: int = 0) -> int:
    """Encode DIV1 while allowing explicit reserved-field negative tests."""
    return (0x1C << 26) | (dividend << 21) | (divisor << 16) | (reserved << 6) | 0x1A
...
async def execute_div1(
    dut,
    fields: tuple[int, int],
    source_values: tuple[int, int],
) -> tuple[int, int]:
    """Execute one legal DIV1 and verify all visible combinational events and state."""
    ...
    expected_hi, expected_lo = expected_div1(dividend_value, divisor_value)
...
@cocotb.test()
async def test_r5900_div1_signed_quotient_remainder_and_low_word_sources(dut) -> None:
    ...
    for dividend_word, divisor_word, expected_hi, expected_lo in cases:
        ...
        assert await execute_div1(dut, (3, 4), (dividend, divisor)) == (
...
@cocotb.test()
async def test_r5900_div1_zero_divisor_has_defined_r5900_results(dut) -> None:
    ...
    for dividend_word, expected_hi, expected_lo in (
        ...
        assert await execute_div1(dut, (8, 0), (dividend, 0)) == (
            expected_hi & SCALAR_MASK,
            expected_lo & SCALAR_MASK,
        )
...
@cocotb.test()
async def test_r5900_div1_overflow_and_pc_wrap(dut) -> None:
    ...
    assert await execute_div1(dut, (31, 30), (dividend, divisor)) == (
        0,
...
@cocotb.test()
async def test_r5900_div1_rejects_nonzero_reserved_rd_or_sa(dut) -> None:
    """Reject every class of noncanonical DIV1 reserved-field population."""
    await initialize(dut, RESERVED_TEST_PC)
    for reserved in (1, 1 << 5, 0x3FF):
        instruction = encode_div1(3, 4, reserved=reserved)
        dut.instruction_i.value = instruction
        dut.instruction_valid_i.value = 1
        await Timer(1, unit="ns")
