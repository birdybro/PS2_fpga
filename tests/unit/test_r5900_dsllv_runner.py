"""Pytest orchestration for directed R5900 DSLLV tests."""
...
@pytest.mark.unit
def test_r5900_dsllv_directed() -> None:
    build = Path(os.environ.get("PS2_BUILD_ROOT", ROOT / "build"))
    runner = get_runner("verilator")
    directory = build / "pytest/r5900_dsllv_directed"
    runner.build(
        sources=sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_dsllv",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=directory,
        test_dir=TEST_DIR,
        seed=int(os.environ.get("RANDOM_SEED", "1")),
        results_xml=str(build / "results/cocotb-r5900-dsllv-directed.xml"),
    )
