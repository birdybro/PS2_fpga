"""Pytest orchestration for behavioral RAM 32-bit byte enables."""
...
@pytest.mark.unit
def test_behavioral_system_ram_byte_enables_with_verilator() -> None:
    """Verify all lower four byte strobes and preserve disabled storage lanes."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_byte_enables"
    results_path = build_root / "results" / "cocotb-behavioral-system-ram-byte-enables.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_byte_enables",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
