"""Directed architectural tests for R5900 ADDU."""
...
def encode_addu(destination: int, source_a: int, source_b: int, shift: int = 0) -> int:
    return (source_a << 21) | (source_b << 16) | (destination << 11) | (shift << 6) | 0x21


def expected_addu(old_destination: int, source_a_value: int, source_b_value: int) -> int:
    word = ((source_a_value & WORD_MASK) + (source_b_value & WORD_MASK)) & WORD_MASK
...
async def execute_addu(
    ...
    instruction = encode_addu(destination, source_a, source_b)
    expected = expected_addu(old_destination, source_a_value, source_b_value)
    ...
    assert int(dut.operation_o.value) == OPERATION_ADDU
...
@cocotb.test()
async def test_r5900_addu_word_wrap_and_sign_extension_boundaries(dut) -> None:
    ...
    for source_a_word, source_b_word, expected_scalar in cases:
        source_a_value = 0xFFFF_FFFF_FFFF_FFFF_1234_5678_0000_0000 | source_a_word
        source_b_value = 0xAAAA_AAAA_AAAA_AAAA_8765_4321_0000_0000 | source_b_word
...
@cocotb.test()
async def test_r5900_addu_handles_both_destination_aliases_and_identical_sources(dut) -> None:
    ...
    await execute_addu(dut, 31, 31, 30, (source_a, source_a, source_b))
    await seed_gpr(dut, 31, source_a)
    await seed_gpr(dut, 30, source_b)
    await execute_addu(dut, 30, 31, 30, (source_b, source_a, source_b))
    await seed_gpr(dut, 29, 0x9999_AAAA_BBBB_CCCC_DDDD_EEEE_FFFF_0000)
    ...
    await execute_addu(
        ...
        28,
        28,
        (0x9999_AAAA_BBBB_CCCC_DDDD_EEEE_FFFF_0000, source_b, source_b),
    )


@cocotb.test()
async def test_r5900_addu_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_a = 0x9999_AAAA_BBBB_CCCC_0123_4567_7FFF_FFFF
    source_b = 0x1111_2222_3333_4444_FEDC_BA98_0000_0001
    await seed_gpr(dut, 8, source_a)
    await seed_gpr(dut, 9, source_b)
    await execute_addu(dut, 0, 8, 9, (0, source_a, source_b))
    assert int(dut.pc_o.value) == 0


@cocotb.test()
async def test_r5900_addu_rejects_nonzero_reserved_shift_field(dut) -> None:
    ...
    instruction = encode_addu(5, 3, 4, shift=1)
    dut.instruction_i.value = instruction
    dut.instruction_valid_i.value = 1
    await Timer(1, unit="ns")
