"""Pytest orchestration for exact zero-word R5900 NOP execution tests."""
...
def nop_sources() -> list[Path]:
    ...
    return [
        ...
        TESTBENCH_DIR / "r5900_nop_top.sv",
    ]


@pytest.mark.unit
def test_r5900_nop_directed() -> None:
    """Verify exact decode, no GPR change, PC advance, wrap, and retirement trace."""
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_nop_directed"
    results_path = build_root / "results" / "cocotb-r5900-nop-directed.xml"
