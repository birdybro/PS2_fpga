"""Directed architectural tests for R5900 SLTIU."""
...
def encode_sltiu(destination: int, source: int, immediate: int) -> int:
    return (0x0B << 26) | (source << 21) | (destination << 16) | immediate


def expected_sltiu(old_destination: int, source_value: int, immediate: int) -> int:
    extended_immediate = immediate | ((SCALAR_MASK ^ 0xFFFF) if immediate & 0x8000 else 0)
    result = int((source_value & SCALAR_MASK) < extended_immediate)
...
async def execute_sltiu(
    ...
    instruction = encode_sltiu(destination, source, immediate)
    expected = expected_sltiu(old_destination, source_value, immediate)
    ...
    assert int(dut.operation_o.value) == OPERATION_SLTIU
...
@cocotb.test()
async def test_r5900_sltiu_sign_extension_then_unsigned_comparison_boundaries(dut) -> None:
    ...
    for source_scalar, immediate, expected_scalar in cases:
        ...
        assert (
            await execute_sltiu(dut, 5, 4, immediate, (old_destination, source_value))
            == destination_upper | expected_scalar
        )


@cocotb.test()
async def test_r5900_sltiu_source_destination_alias_and_zero_source(dut) -> None:
    await initialize(dut, 0x2000)
    aliased = 0xCAFE_BABE_1234_5678_0000_0000_0000_8000
    await seed_gpr(dut, 31, aliased)
    await execute_sltiu(dut, 31, 31, 0x8000, (aliased, aliased))
    zero_source_destination = 0x1111_2222_3333_4444_AAAA_BBBB_CCCC_DDDD
    await seed_gpr(dut, 7, zero_source_destination)
    await execute_sltiu(dut, 7, 0, 0xFFFF, (zero_source_destination, 0))


@cocotb.test()
async def test_r5900_sltiu_to_zero_retires_without_writeback_and_wraps_pc(dut) -> None:
    await initialize(dut, 0xFFFF_FFFC)
    source_value = 0x9999_AAAA_BBBB_CCCC_0000_0000_0000_8000
    await seed_gpr(dut, 9, source_value)
    await execute_sltiu(dut, 0, 9, 0x8000, (0, source_value))
