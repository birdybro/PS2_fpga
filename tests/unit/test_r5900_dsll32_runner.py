"""Pytest orchestration for directed R5900 DSLL32 tests."""
...
@pytest.mark.unit
def test_r5900_dsll32_directed() -> None:
    build = Path(os.environ.get("PS2_BUILD_ROOT", ROOT / "build"))
    runner = get_runner("verilator")
    directory = build / "pytest/r5900_dsll32_directed"
    runner.build(
        sources=sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_dsll32",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=directory,
        test_dir=TEST_DIR,
        results_xml=str(build / "results/cocotb-r5900-dsll32-directed.xml"),
    )
