"""Pytest orchestration for differential 64-bit behavioral RAM reads."""
...
def test_behavioral_system_ram_read64_against_python_model() -> None:
    """Compare all aligned doublewords with an independent byte model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_read64_differential"
    results_path = build_root / "results" / "cocotb-ram-read64-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_read64_differential",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
