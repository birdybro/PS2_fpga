"""Pytest orchestration for randomized differential R5900 SRLV verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_srlv"


def srlv_sources() -> list[Path]:
    """Return the ordered RTL and shared harness sources for SRLV execution."""
    return [
...
def test_r5900_srlv_randomized_differential() -> None:
    """Compare SRLV PC, all GPRs, writeback, and retirement to the Python model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_srlv_differential"
    results_path = build_root / "results" / "cocotb-r5900-srlv-differential.xml"
    runner = get_runner("verilator")
    runner.build(
        sources=srlv_sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_srlv_differential",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=build_dir,
