"""Pytest orchestration for exhaustive differential 128-bit RAM writes."""
...
def test_behavioral_system_ram_write128_against_python_model() -> None:
    """Compare every strobe mask and the complete resulting RAM image."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_write128_differential"
    results_path = build_root / "results" / "cocotb-ram-write128-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_write128_differential",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
