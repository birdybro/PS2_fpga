"""Pytest orchestration for differential 64-bit behavioral RAM writes."""
...
def test_behavioral_system_ram_write64_against_python_model() -> None:
    """Compare all byte-enable patterns and the complete resulting RAM image."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_write64_differential"
    results_path = build_root / "results" / "cocotb-ram-write64-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_write64_differential",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
