"""Pytest orchestration for differential behavioral RAM byte enables."""
...
def test_behavioral_system_ram_byte_enables_against_python_model() -> None:
    """Compare byte-enabled RTL writes with an independent full-image model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_byte_enables_differential"
    results_path = build_root / "results" / "cocotb-ram-byte-enables-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_byte_enables_differential",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
