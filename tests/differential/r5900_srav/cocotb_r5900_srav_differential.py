"""Randomized differential verification for R5900 32-bit SRAV."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_srav
...
async def execute_case(
    ...
    """Execute and compare one complete differential SRAV transition."""
    destination, source, shift_register = fields
    instruction = encode_srav(destination, source, shift_register)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_srav_against_python_architectural_model(dut) -> None:
    """Compare all state and events over boundary and seeded SRAV cases."""
