"""Randomized differential verification for R5900 32-bit SRA."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_sra
...
async def execute_case(
    ...
    """Execute and compare one complete differential SRA transition."""
    destination, source, shift_amount = fields
    instruction = encode_sra(destination, source, shift_amount)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_sra_against_python_architectural_model(dut) -> None:
    """Compare all GPRs, PC, writeback, and retirement over seeded SRA cases."""
