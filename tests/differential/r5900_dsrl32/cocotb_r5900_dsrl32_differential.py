"""Randomized differential verification for R5900 DSRL32."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_dsrl32
...
@cocotb.test()
async def test_dsrl32_against_python_model(dut) -> None:
    ...
    for iteration, (destination, source, count) in enumerate((*boundaries, *random_cases)):
        instruction = encode_dsrl32(destination, source, count)
