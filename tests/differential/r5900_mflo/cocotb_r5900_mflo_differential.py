"""Randomized differential verification for R5900 MFLO."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_mflo
...
async def seed_hilo(dut, model: R5900State, lo_value: int) -> R5900State:
    """Seed a new randomized LO while preserving the other accumulator fields."""
    ...
    await edge(dut)
    return model.write_lo(lo_value)
...
async def execute_case(
    ...
    """Execute and compare one complete differential MFLO transition."""
    instruction = encode_mflo(destination)
    old_pc = model.pc
    old_destination = model.gprs[destination]
    expected_value = (old_destination & ~SCALAR_MASK) | model.lo
...
@cocotb.test()
async def test_r5900_mflo_against_python_architectural_model(dut) -> None:
    """Compare all state and events over randomized LO values and destinations."""
    ...
    for iteration, (destination, lo_value) in enumerate((*boundary_cases, *random_cases)):
        model = await seed_hilo(dut, model, lo_value)
