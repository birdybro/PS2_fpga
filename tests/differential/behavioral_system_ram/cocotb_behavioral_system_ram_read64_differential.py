"""Differential aligned 64-bit behavioral RAM reads."""
...
async def rtl_read64(dut, address: int) -> int:
    """Issue and consume one aligned 64-bit bus read."""
...
@cocotb.test()
async def every_aligned_doubleword_matches_independent_byte_model(dut) -> None:
    """Compare every aligned doubleword after asymmetric byte initialization."""
    ...
    for address in range(RAM_SIZE):
        value = ((address * 131) ^ (address >> 2) ^ 0x6D) & 0xFF
