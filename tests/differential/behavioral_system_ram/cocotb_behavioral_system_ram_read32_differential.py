"""Differential aligned 32-bit behavioral RAM reads."""
...
async def rtl_read32(dut, address: int) -> int:
    """Issue and consume one aligned 32-bit bus read."""
...
@cocotb.test()
async def every_aligned_word_matches_independent_byte_model(dut) -> None:
    """Compare every aligned word after initializing all bytes asymmetrically."""
    ...
    for address in range(RAM_SIZE):
        value = ((address * 73) ^ (address >> 1) ^ 0xA5) & 0xFF
