"""Randomized differential verification for R5900 32-bit SRLV."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_srlv
...
async def execute_case(
    ...
    """Execute and compare one complete differential SRLV transition."""
    destination, source, shift_register = fields
    instruction = encode_srlv(destination, source, shift_register)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_srlv_against_python_architectural_model(dut) -> None:
    """Compare all state and events over boundary and seeded SRLV cases."""
