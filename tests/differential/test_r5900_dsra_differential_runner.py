"""Pytest orchestration for randomized differential R5900 DSRA verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_dsra"


def dsra_sources() -> list[Path]:
    """Return ordered RTL and harness sources for DSRA execution."""
    return [
...
def test_r5900_dsra_randomized_differential() -> None:
    """Compare DSRA PC, all GPRs, writeback, and retirement to the Python model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_dsra_differential"
    results_path = build_root / "results" / "cocotb-r5900-dsra-differential.xml"
    runner = get_runner("verilator")
    runner.build(
        sources=dsra_sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_dsra_differential",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=build_dir,
