"""Pytest orchestration for differential 32-bit behavioral RAM writes."""
...
def test_behavioral_system_ram_write32_against_python_model() -> None:
    """Compare all full-word RTL writes and reads with an independent model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "behavioral_system_ram_write32_differential"
    results_path = build_root / "results" / "cocotb-ram-write32-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_behavioral_system_ram_write32_differential",
        hdl_toplevel="behavioral_system_ram_bus_top",
        build_dir=build_dir,
