"""Randomized differential verification for R5900 32-bit SRL."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_srl
...
async def execute_case(
    ...
    """Execute and compare one complete differential SRL transition."""
    destination, source, shift_amount = fields
    instruction = encode_srl(destination, source, shift_amount)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_srl_against_python_architectural_model(dut) -> None:
    """Compare all GPRs, PC, writeback, and retirement over seeded SRL cases."""
