"""Pytest orchestration for randomized differential R5900 ADDIU verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_addiu"


@pytest.mark.differential
@pytest.mark.randomized
def test_r5900_addiu_randomized_differential() -> None:
    sources = [
    ...
    build_dir = build_root / "pytest" / "r5900_addiu_differential"
    ...
    result = runner.test(
        test_module="cocotb_r5900_addiu_differential",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=build_dir,
        test_dir=TESTBENCH_DIR,
        results_xml=str(build_root / "results/cocotb-r5900-addiu-differential.xml"),
    )
