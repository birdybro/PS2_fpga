"""Randomized differential verification for R5900 64-bit DSRA."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_dsra
...
async def execute_case(
    ...
    """Execute and compare one complete differential DSRA transition."""
    destination, source, shift_amount = fields
    instruction = encode_dsra(destination, source, shift_amount)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_dsra_against_python_architectural_model(dut) -> None:
    """Compare all GPRs, PC, writeback, and retirement over seeded DSRA cases."""
