"""Randomized differential verification for R5900 unsigned word DIVU1."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_divu1
...
def expected_divu1(dividend_value: int, divisor_value: int) -> tuple[int, int]:
    """Calculate DIVU1 results independently from the architectural model."""
...
async def execute_case(
    ...
    """Execute and compare one complete differential DIVU1 transition."""
    dividend_register, divisor_register = fields
    instruction = encode_divu1(dividend_register, divisor_register)
    expected_hi, expected_lo = expected_divu1(
...
@cocotb.test()
async def test_r5900_divu1_against_python_architectural_model(dut) -> None:
    """Compare all architectural state and events over 524 DIVU1 operations."""
