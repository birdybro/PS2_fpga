"""Pytest orchestration for randomized differential R5900 MFLO verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_mflo"


def mflo_sources() -> list[Path]:
    """Return ordered RTL and harness sources for MFLO execution."""
    return [
...
def test_r5900_mflo_randomized_differential() -> None:
    """Compare MFLO state and events against the Python architectural model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_mflo_differential"
    results_path = build_root / "results" / "cocotb-r5900-mflo-differential.xml"
    runner = get_runner("verilator")
    runner.build(
        sources=mflo_sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_mflo_differential",
        hdl_toplevel="r5900_mult_top",
        build_dir=build_dir,
