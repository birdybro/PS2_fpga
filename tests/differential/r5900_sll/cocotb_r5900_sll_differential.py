"""Randomized differential verification for R5900 32-bit SLL."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_sll
...
def require_non_nop(fields: tuple[int, int, int]) -> tuple[int, int, int]:
    """Keep this SLL-specific stream distinct from exact zero-word NOP coverage."""
    return (0, 0, 1) if fields == (0, 0, 0) else fields
...
async def execute_case(
    ...
    """Execute and compare one complete differential SLL transition."""
    destination, source, shift_amount = fields
    instruction = encode_sll(destination, source, shift_amount)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_sll_against_python_architectural_model(dut) -> None:
    """Compare all GPRs, PC, writeback, and retirement over seeded SLL cases."""
    ...
    cases = (require_non_nop(fields) for fields in (*boundary_cases, *random_cases))
    for iteration, fields in enumerate(cases):
