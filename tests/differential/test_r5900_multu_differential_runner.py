"""Pytest orchestration for randomized differential R5900 MULTU verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_multu"


def multu_sources() -> list[Path]:
    """Return ordered RTL and harness sources for MULTU execution."""
    return [
...
def test_r5900_multu_randomized_differential() -> None:
    """Compare MULTU state and events against the Python architectural model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_multu_differential"
    results_path = build_root / "results" / "cocotb-r5900-multu-differential.xml"
    runner = get_runner("verilator")
    runner.build(
        sources=multu_sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_multu_differential",
        hdl_toplevel="r5900_mult_top",
        build_dir=build_dir,
