"""Randomized differential verification for R5900 unsigned word MULTU1."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_multu1
...
def expected_product(source_a: int, source_b: int) -> tuple[int, int]:
    """Calculate unsigned product with independently sign-extended halves."""
    product = ((source_a & WORD_MASK) * (source_b & WORD_MASK)) & SCALAR_MASK
...
async def execute_case(
    ...
    """Execute and compare one complete differential MULTU1 transition."""
...
@cocotb.test()
async def test_r5900_multu1_against_python_architectural_model(dut) -> None:
    """Compare PC, GPRs, both accumulator lanes, and events over MULTU1 cases."""
