"""Randomized differential verification for R5900 MFHI1."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_mfhi1
...
async def seed_hi1(dut, model: R5900State, hi1_value: int) -> R5900State:
    """Seed a new randomized HI1 while preserving the other accumulator fields."""
    dut.seed_hi_i.value = model.hi
    ...
    await edge(dut)
    return model.write_hi1(hi1_value)
...
async def execute_case(
    ...
    """Execute and compare one complete differential MFHI1 transition."""
    instruction = encode_mfhi1(destination)
    old_pc = model.pc
    old_destination = model.gprs[destination]
    expected_value = (old_destination & ~SCALAR_MASK) | model.hi1
...
@cocotb.test()
async def test_r5900_mfhi1_against_python_architectural_model(dut) -> None:
    """Compare all state and events over randomized HI1 values and destinations."""
    ...
    for iteration, (destination, hi1_value) in enumerate((*boundary_cases, *random_cases)):
        model = await seed_hi1(dut, model, hi1_value)
