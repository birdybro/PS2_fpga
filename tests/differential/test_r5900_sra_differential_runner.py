"""Pytest orchestration for randomized differential R5900 SRA verification."""
...
def test_r5900_sra_randomized_differential() -> None:
    """Compare SRA PC, all GPRs, writeback, and retirement to the Python model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_sra_differential"
    results_path = build_root / "results" / "cocotb-r5900-sra-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_r5900_sra_differential",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=build_dir,
