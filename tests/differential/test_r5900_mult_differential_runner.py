"""Pytest orchestration for randomized differential R5900 MULT verification."""
...
def mult_sources() -> list[Path]:
    """Return ordered RTL and harness sources for MULT execution."""
    return [
...
def test_r5900_mult_randomized_differential() -> None:
    """Compare MULT state and events against the Python architectural model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_mult_differential"
    results_path = build_root / "results" / "cocotb-r5900-mult-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_r5900_mult_differential",
        hdl_toplevel="r5900_mult_top",
        build_dir=build_dir,
