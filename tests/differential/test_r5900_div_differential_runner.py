"""Pytest orchestration for randomized differential R5900 DIV verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_div"


def div_sources() -> list[Path]:
    """Return ordered RTL and harness sources for DIV execution."""
    return [
...
def test_r5900_div_randomized_differential() -> None:
    """Compare DIV state and events against the Python architectural model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_div_differential"
    results_path = build_root / "results" / "cocotb-r5900-div-differential.xml"
    ...
    result = runner.test(
        test_module="cocotb_r5900_div_differential",
        hdl_toplevel="r5900_mult_top",
        build_dir=build_dir,
