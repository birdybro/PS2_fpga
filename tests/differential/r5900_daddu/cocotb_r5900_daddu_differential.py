"""Randomized differential verification for R5900 DADDU."""
...
@cocotb.test()
async def test_r5900_daddu_against_python_model(dut) -> None:
    ...
    for iteration, (destination, source_a, source_b) in enumerate((*boundaries, *random_cases)):
        ...
        old_pc = model.pc
        result_scalar = (
            (model.gprs[source_a] & SCALAR_MASK) + (model.gprs[source_b] & SCALAR_MASK)
        ) & SCALAR_MASK
