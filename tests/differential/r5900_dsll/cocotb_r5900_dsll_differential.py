"""Randomized differential verification for R5900 64-bit DSLL."""
...
from reference.ee.r5900 import GPR_COUNT, GPR_WIDTH, R5900State, encode_dsll
...
async def execute_case(
    ...
    """Execute and compare one complete differential DSLL transition."""
    destination, source, shift_amount = fields
    instruction = encode_dsll(destination, source, shift_amount)
    old_pc = model.pc
...
@cocotb.test()
async def test_r5900_dsll_against_python_architectural_model(dut) -> None:
    """Compare all GPRs, PC, writeback, and retirement over seeded DSLL cases."""
