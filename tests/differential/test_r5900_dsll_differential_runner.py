"""Pytest orchestration for randomized differential R5900 DSLL verification."""
...
TESTBENCH_DIR = Path(__file__).resolve().parent / "r5900_dsll"


def dsll_sources() -> list[Path]:
    """Return ordered RTL and harness sources for DSLL execution."""
    return [
...
def test_r5900_dsll_randomized_differential() -> None:
    """Compare DSLL PC, all GPRs, writeback, and retirement to the Python model."""
    seed = int(os.environ.get("RANDOM_SEED", "1"))
    build_root = Path(os.environ.get("PS2_BUILD_ROOT", REPO_ROOT / "build"))
    build_dir = build_root / "pytest" / "r5900_dsll_differential"
    results_path = build_root / "results" / "cocotb-r5900-dsll-differential.xml"
    runner = get_runner("verilator")
    runner.build(
        sources=dsll_sources(),
    ...
    result = runner.test(
        test_module="cocotb_r5900_dsll_differential",
        hdl_toplevel="r5900_shift_immediate_top",
        build_dir=build_dir,
