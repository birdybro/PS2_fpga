...
canonical 32-bit shifts, LUI, ORI, ANDI, and XORI are implemented with PC
advance and retirement trace. No other instruction executes. Illegal words emit
a functional diagnostic and are suppressed before execution, but do not enter
an architectural exception; COP0 remains unimplemented.
...
NOP, all six 32-bit shifts, LUI, ORI, ANDI, and XORI are complete; the other 11
entries in the initial R5900 ISA coverage matrix remain pending. The matrix
records planned encodings and ownership; each remaining row must pass its
